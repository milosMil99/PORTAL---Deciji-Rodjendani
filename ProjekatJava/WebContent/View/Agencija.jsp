...
<body>
 <!-- Navigation-->
        ...
        <% Agencija agencija = (Agencija) request.getAttribute("agencija");
        	ArrayList<Ponuda> listaPonuda = (ArrayList<Ponuda>) request.getAttribute("ponude");%>
        ...
        <section class="page-section  text-black mb-0" id="about">
            <div class="container">
                ...
                <h2 class="page-section-heading text-center text-uppercase text-black"><%= agencija.getNaziv() %></h2>
                <!-- Icon Divider-->
                ...
                <div class="row">
                    <div class="col-lg-4 ms-auto">
                    	<img class="img-fluid" src="img\<%= agencija.getSlika()%>">
                    </div>
                    	...
                    	<table class="table">
						  <thead>
						    <tr>
						      ...
						      <th scope="col"></th>
						      <th scope="col"><%= agencija.getLokacija() %></th>
						  ...
						  <thead>
						    <tr>
						      ...
						      <th scope="col"></th>
						      <th scope="col"><%=agencija.getPocetakRadnog()%> - <%=agencija.getKrajRadnog() %></th>	      
						    </tr>
						  </thead>
						</table>
						<form action="ServletRezervacija" method="GET"> 
							<input type="hidden" name="AgencijaID" value="<%=agencija.getAgencijaID() %>">
                    			<%
		                    	for(Ponuda p : listaPonuda)
		                    	{
		                    		if(p.GetAktivan() == true)
		                    		{
		                    			%>
			                    		<div>
		                    				<input type="radio" name="ponuda" value="<%= p.getPonudaID() %>" checked="checked" /> <%= p.getNaziv()%> (<%= p.getOpis() %>) Cena: <%= p.getCena() %> <br>
		                    			</div>
			                    		<%
		                    		}
	                    	...
	                    	%>
	                    		...
	                    		<div class="text-center mt-4">
				                    <input type="submit" class="btn btn-xl btn-outline-dark" value="Rezevrisi" >
				                </div>
	                    </form>
	                    	</div>
